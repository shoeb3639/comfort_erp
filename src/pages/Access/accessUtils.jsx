...
export function SystemBadge({ enabled }) {
  return (
    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ring-1 ring-inset ${
      enabled ? 'bg-brand-50 text-brand-700 ring-brand-600/20' : 'bg-slate-100 text-slate-600 ring-slate-500/20'
    }`}>
...
export function TableShell({ columns, children, empty = false, minWidth = '1000px' }) {
  return (
    <div className="overflow-x-auto rounded-xl border border-slate-200">
      ...
      {empty && <div className="bg-white px-4 py-10 text-center text-sm text-slate-500">No records found.</div>}
    </div>
  )
}

export function PermissionKey({ value }) {
  return <code className="rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700">{value}</code>
}
