...
function formatPeriod(item) {
  if (item.dateType === "range") {
    return [formatDate(item.serviceStartDate), formatDate(item.serviceEndDate)]
      .filter(Boolean)
      .join(" to ");
  }
...
function SummaryLine({ label, value, strong = false }) {
  return (
    <div
      className={`grid grid-cols-[63%_37%] items-center border-t ${strong ? "border-b border-black font-bold" : "border-slate-300 font-medium"}`}
    >
      <div className="px-3 py-1 text-right">{label}</div>
      <div className="min-w-0 whitespace-nowrap border-l border-slate-300 px-2 py-1 text-right text-[13px]">
        {amount(value)}
      </div>
    </div>
  );
...
function InvoicePreviewTemplate({ invoice }) {
  ...
  const company = invoice.companyDetails || companyDetails;
  ...
  const isDirectInvoice =
    (invoice.invoice_source || invoice.invoiceSource) === "direct";
  const serviceCity =
    invoice.serviceCity || invoice.service_city || invoice.placeOfSupply || "-";
  const bookingOrReference = isDirectInvoice
    ...
    <article className="mx-auto min-h-[297mm] w-[210mm] max-w-full border border-[#ddd] bg-white px-[42px] pb-[36px] pt-[42px] font-['Lato',Arial,Helvetica,sans-serif] text-[14px] font-normal leading-[1.5] text-black print:min-h-[297mm] print:w-[210mm] print:max-w-none print:px-[42px] print:pb-[36px] print:pt-[42px]">
      <div className="border border-black">
        <div className="grid grid-cols-3 items-center border-b border-black px-3 py-2.5 leading-[1.5]">
          ...
          <div className="text-center font-extrabold uppercase tracking-wide">
            Tax Invoice
          </div>
          <div />
        </div>
        ...
        <header className="grid grid-cols-[64%_36%] border-b border-black">
          ...
          <div className="border-l border-black px-4 py-3 leading-[1.5]">
            ...
            <p className="mt-1 uppercase">
              {invoice.billingAddress || "Billing address not available"}
            </p>
            <p className="mt-1.5 font-bold">
              GST No: {invoice.customerGstin || "Not applicable"}
            </p>
          </div>
        </header>
        ...
        <section className="grid grid-cols-4 border-b border-black text-[13px] leading-[1.4]">
          ...
          <div className="border-r border-black px-3 py-2">
            <span className="font-semibold">
              {isDirectInvoice ? "Reference:" : "Booking ID:"}
            </span>{" "}
            {bookingOrReference}
          </div>
          <div className="border-r border-black px-3 py-2">
        ...
        <table className="w-full table-fixed border-collapse border-y border-slate-300 bg-white leading-[1.5]">
          <thead className="bg-slate-100">
            <tr className="font-bold uppercase text-slate-950">
              <th className="w-[20%] whitespace-nowrap px-3 py-2 text-left">
                Date of Use
              </th>
              ...
              <th className="w-[12%] border-l border-slate-300 px-4 py-2.5 text-right">
                Rate
              </th>
              <th className="w-[14%] border-l border-slate-300 px-2 py-2.5 text-right">
                Amount
              </th>
            </tr>
          </thead>
          <tbody>
            {(invoice.items || []).map((item, index) => (
              <tr
                key={item.id}
                className={`align-top border-t border-slate-200 ${index % 2 === 1 ? "bg-slate-50" : "bg-white"}`}
              >
                <td className="px-3 py-2 font-medium">
                  <ServicePeriod item={item} />
                </td>
                ...
                <td className="border-l border-slate-200 px-4 py-2.5 text-right font-medium">
                  {cleanMoney(item.rate)}
                </td>
                <td className="border-l border-slate-200 px-2 py-2.5 text-right font-bold">
                  {cleanMoney(item.amount)}
                </td>
              </tr>
            ))}
            <tr className="border-y-2 border-black bg-slate-100 font-extrabold">
              ...
              <td className="whitespace-nowrap border-l border-slate-300 px-2 py-1.5 text-right text-[13px]">
        ...
        <section className="grid grid-cols-[62%_38%] border-b border-slate-300 bg-white leading-[1.5]">
          ...
          <div className="border-l border-slate-300">
            ...
            {totals.igst > 0 && (
              <SummaryLine label="IGST 5%" value={totals.igst} />
            )}
            ...
            <div className="px-4 pb-4 pt-8 text-right">
              ...
              />
              <p className="mt-1 font-bold leading-[1.5]">
                AUTHORISED SIGNATORY FOR
              </p>
              <p className="font-medium leading-[1.5]">{company.name}</p>
            </div>
          </div>
        </section>
      </div>
