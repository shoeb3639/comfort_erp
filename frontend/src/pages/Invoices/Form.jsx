...
} from "./components/InvoiceItemTable";
import {
    ...
    <div className="text-sm">
      ...
      <div className="space-y-1">
        {rows.map(([label, value]) => (
          <div
            key={label}
            className={`flex justify-between gap-4 py-1 ${value || label.includes("Amount") || gstType === "No GST" ? "" : "text-slate-400"}`}
          >
            <span>{label}</span>
...
function createDefaultInvoiceValues(settings, invoice) {
  ...
  return {
    ...
    customerType:
      invoice?.customerType || invoice?.customer_type || "Individuals",
    customerMode: "existing",
    directCustomerId:
      invoice?.directCustomerId || invoice?.billing_customer_id || "",
    directTravellerId:
      invoice?.directTravellerId || invoice?.traveller_id || "",
    newCustomerName: "",
    newContactPerson: "",
    newMobile: "",
    newEmail: "",
    ...
    referenceNumber:
      invoice?.referenceNumber || invoice?.reference_number || "",
    placeOfSupply:
    ...
    includeVehiclePnL:
      invoice?.includeVehiclePnL === true ||
      invoice?.includeVehiclePnL === "Yes"
        ? "Yes"
        : "No",
...
function InvoiceFormPage() {
  ...
  const directCustomerOptions = customers.filter(
    (customer) => customer.type === values.customerType,
  );
  const directTravellerOptions = travellers.filter(
    (traveller) =>
      traveller.customer_id === values.directCustomerId &&
  ...
  function handleBookingChange(bookingId) {
    ...
    setValue(
      "billingType",
      isPackage ? "Package" : isLocal ? "Local" : "Outstation",
    );
    ...
    setValue(
      "vehicle",
      `${booking.vehicleType || ""} ${booking.vehicleRegistrationNo || ""}`.trim(),
    );
  ...
  function handleDirectCustomerChange(customerId) {
    ...
    const firstTraveller = travellers.find(
      (traveller) =>
        traveller.customer_id === customerId && traveller.status === "Active",
    );
    setValue(
      "billingName",
      customer.type === "Individuals"
        ? customer.displayName || customer.name
        : firstTraveller?.name || customer.contacts?.[0]?.name || "",
    );
    setValue(
      "billingCustomer",
      customer.billingName || customer.displayName || customer.name || "",
    );
    setValue("billingAddress", customer.address || "");
    setValue("customerGstin", customer.gstin || "");
  ...
  function handleVehicleChange(vehicleId) {
    ...
    if (vehicle)
      setValue(
        "vehicle",
        `${vehicle.type || ""} ${vehicle.plate || ""}`.trim(),
      );
  }
  ...
  async function createDirectCustomerIfNeeded(formValues) {
    ...
    if (formValues.customerMode === "existing") {
      return {
        ...formValues,
        billing_customer_id: formValues.directCustomerId,
      };
    }
    ...
    const billingName =
      formValues.customerType === "Individuals"
        ? name
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit((formValues) =>
        persistInvoice(formValues, defaultSaveStatus),
      )}
    >
      {lastDraftId && (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
          {isEditMode
            ? `Invoice ${lastDraftId} updated.`
            : `Draft saved as ${lastDraftId}. Invoice number was not consumed.`}
        </div>
      )}
      ...
      <article className="w-full bg-white p-5 font-['Lato',Arial,Helvetica,sans-serif] text-[14px] leading-6 text-black shadow-sm print:w-full print:p-0 print:shadow-none">
        <div className="border border-black">
          <div className="border-b border-black p-[5px] text-center text-[18px] font-bold uppercase leading-6">
            Tax Invoice
          </div>
            ...
            <div className="grid gap-3 lg:grid-cols-[220px_1fr]">
              <label className="block">
                ...
                <select
                  ...
                  onChange={(event) =>
                    handleInvoiceSourceChange(event.target.value)
                  }
                >
                      ...
                      <label className="block lg:col-span-2">
                        ...
                        <select
                          ...
                          {...register("directCustomerId", {
                            validate: (value) =>
                              isDirectInvoice && !isNewCustomerMode && !value
                                ? "Billing customer is required"
                                : true,
                          })}
                          onChange={(event) =>
                            handleDirectCustomerChange(event.target.value)
                          }
                        >
                        ...
                        {errors.directCustomerId && (
                          <span className="mt-1 block text-xs font-semibold text-rose-600">
                            {errors.directCustomerId.message}
                          </span>
                        )}
                      </label>
                      {(values.customerType === "Corporate" ||
                        values.customerType === "Travel Agent") && (
                        <div className="block lg:col-span-2">
                          ...
                          <div className="mt-1 flex gap-2">
                            <select
                              ...
                              onChange={(event) =>
                                handleDirectTravellerChange(event.target.value)
                              }
                            >
                              <option value="">
                                Optional - select traveller
                              </option>
                    ...
                    <>
                      <label className="block lg:col-span-2">
                        <span className={labelClass}>
                          {values.customerType === "Individuals"
                            ? "Name"
                            : values.customerType === "Corporate"
                              ? "Company Name"
                              : "Agent / Company Name"}
                      ...
                      <label className="block lg:col-span-4">
                        ...
                        />
                        {errors.newBillingAddress && (
                          <span className="mt-1 block text-xs font-semibold text-rose-600">
                            {errors.newBillingAddress.message}
                          </span>
                        )}
                      </label>
                    </>
                    ...
                    <EditableField label="Vehicle">
                      <select
                        ...
                        {...register("vehicleId", {
                          validate: (value) =>
                            isDirectInvoice &&
                            values.includeVehiclePnL === "Yes" &&
                            !value
                              ? "Vehicle is required for Vehicle P&L"
                              : true,
                        })}
                        onChange={(event) =>
          ...
          {isDirectInvoice && (
            <section className="grid gap-2 border-b border-black px-[15px] py-[5px] md:grid-cols-3">
              <EditableField label="Service Period">
                <input
                  className={fieldClass}
                  placeholder="18-07-2026 or 27-06-2026 to 29-06-2026"
              ...
              <EditableField label="From Location">
                <input
                  className={fieldClass}
                  {...register("directFromLocation")}
                />
              </EditableField>
              <EditableField label="To Location">
                <input
                  className={fieldClass}
                  {...register("directToLocation")}
                />
              </EditableField>
            </section>
          )}
            ...
            <div className="px-2.5 pb-2.5 pt-0">
              ...
              <div className="mt-[15px] text-right">
                <InvoiceSignature
                  ...
                  className="ml-auto h-[100px] w-[150px] object-contain"
                />
              </div>
            </div>
          </section>
      ...
      <div className="sticky bottom-4 z-10 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-lg backdrop-blur print:hidden sm:flex-row sm:items-center sm:justify-end">
        ...
        <button
          ...
          onClick={handleSubmit((formValues) =>
            persistInvoice(formValues, defaultSaveStatus, "preview"),
          )}
        >
          Preview
        </button>
        <button
          ...
          className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
          onClick={handleSubmit((formValues) =>
            persistInvoice(formValues, "Generated", "print"),
          )}
        >
          Print
        </button>
        ...
        <button
          ...
          onClick={handleSubmit((formValues) =>
            persistInvoice(formValues, "Generated", "preview"),
          )}
        >
          {primaryActionLabel}
        </button>
