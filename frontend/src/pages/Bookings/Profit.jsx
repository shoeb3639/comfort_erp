...
function getBillingValues(booking, invoice) {
  ...
  const otherRecoverableCharges = toNumber(
    closeDetails.otherRecoverableCharges,
  );
...
function getProfitValues(booking, billing) {
  ...
  const netVehicleProfit = isVendorVehicle
    ? 0
    : vehicleRevenue - dieselCost - directVehicleExpense - driverCost;
  const vendorPayableAmount = toNumber(
    closeDetails.vendorPayableAmount || booking?.vendorPayableAmount,
  );
...
function AmountLine({ label, value, strong = false, tone = "default" }) {
  ...
  return (
    <div
      className={`flex items-center justify-between gap-4 py-2 ${strong ? "border-t border-slate-200 pt-3 text-base font-bold" : "text-sm"}`}
    >
...
function BookingProfitPage() {
  ...
  if (!booking) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-600 shadow-sm">
        {loadError || "Booking profit was not found."}{" "}
  ...
  const totalRecoverableCharges =
    billing.tollTax +
    billing.parking +
    billing.driverAllowance +
    billing.otherRecoverableCharges +
    billing.gst;
  const expenseRows = profit.isVendorVehicle
    ? []
    : getExpenseRows(booking, [], profit);
  const route = [
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        ...
        />
        <MetricCard
          label="Total Recoverable Charges"
          value={totalRecoverableCharges}
          tone="warning"
        />
        <MetricCard
          label={profit.isVendorVehicle ? "Vendor Payable" : "Diesel Cost"}
          value={
            profit.isVendorVehicle
              ? profit.finalVendorPayable
              : profit.dieselCost
          }
          tone="danger"
        />
        ...
        <MetricCard
          ...
          suffix="%"
          tone={profit.profitMargin >= 0 ? "success" : "danger"}
        />
      </div>
      ...
      <Section title="Booking Summary">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <ReadOnlyField
            ...
            value={profit.isVendorVehicle ? "Vendor Vehicle" : "Own Vehicle"}
          />
          {profit.isVendorVehicle && (
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Section title="Billing Breakdown">
          ...
          <AmountLine
            ...
            value={billing.driverAllowance}
          />
          <AmountLine
            label="Other Recoverable Charges"
            value={billing.otherRecoverableCharges}
          />
        ...
        {!profit.isVendorVehicle && (
          <Section title="Profit Breakdown">
            <div className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">
              Toll Tax, Parking, Driver Allowance and GST are pass-through
              charges and are not treated as vehicle profit.
            </div>
            <div className="mt-3">
              <AmountLine
                label="Vehicle Revenue = Base Fare"
                value={profit.vehicleRevenue}
              />
              <AmountLine label="Diesel Cost" value={profit.dieselCost} />
              <AmountLine
                label="Direct Vehicle Expense"
                value={profit.directVehicleExpense}
              />
              <AmountLine label="Driver Cost" value={profit.driverCost} />
              <AmountLine
                label="Net Vehicle Profit"
                value={profit.netVehicleProfit}
                strong
                tone="profit"
              />
            </div>
        ...
        {profit.isVendorVehicle && (
          <Section title="Vendor Profit Breakdown">
            <div className="rounded-lg bg-sky-50 px-3 py-2 text-xs font-medium text-sky-800">
              Vendor booking profit is separate revenue margin and is not
              included in own vehicle profit.
            </div>
            <div className="mt-3">
              ...
              <AmountLine
                ...
                value={profit.vendorExtraCharges}
              />
              <AmountLine
                label="Vendor Deduction"
                value={profit.vendorDeduction}
              />
              <AmountLine
                label="Final Vendor Payable"
                value={profit.finalVendorPayable}
              />
              <AmountLine
                label="Vendor Booking Profit"
                value={profit.vendorBookingProfit}
                strong
                tone="profit"
              />
            </div>
      ...
      {!profit.isVendorVehicle && (
        <Section title="Expense Linked to Booking">
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            ...
            {expenseRows.length === 0 && (
              <div className="bg-white px-4 py-10 text-center text-sm text-slate-500">
                No direct diesel, vehicle expense, or driver cost is linked to
                this booking yet.
              </div>
            )}
          </div>
            ...
            <table className="min-w-[760px] divide-y divide-slate-200 text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                <tr>
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    {booking.vendorRateType || "-"}
                  </td>
                  <td className="px-4 py-3 font-semibold text-slate-900">
                    ₹ {money(profit.finalVendorPayable)}
                  </td>
                  <td className="px-4 py-3 text-slate-600">
                    {booking.vendorNotes || "-"}
                  </td>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-2 text-sm text-slate-700">
              {booking.closeDetails?.remarks ||
                booking.notes ||
                "No closing remarks added."}
            </p>
          </div>
          <div className="rounded-xl border border-dashed border-slate-300 bg-white p-4">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Audit history will show close, collection, deposit verification,
              and profit revisions after API integration.
            </p>
          </div>
        </div>
      </Section>
