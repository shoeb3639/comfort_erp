...
function PlatformUsersPage() {
  ...
  useEffect(() => {
    let active = true;
    getPlatformUsers({ page: pagination.page, limit: pagination.limit })
      .then((result) => {
        if (active) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <SummaryCard
          ...
          value={users.length}
          caption={loading ? "Loading…" : "Persisted users"}
        />
        <SummaryCard
          label="Active Users"
        ...
        <SummaryCard
          label="User Scope"
          value="PLATFORM"
          caption="Separate from tenant users"
        />
      </div>
      <Section
        title="Platform User List"
        subtitle="Cablix administrators stored in the platform user directory."
      >
        <TableShell
          columns={[
            "Name",
