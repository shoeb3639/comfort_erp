...
function PaymentsPage() {
  ...
  useEffect(() => {
    let active = true;
    getSubscriptionPayments({ page: pagination.page, limit: pagination.limit })
      .then((result) => {
        if (active) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <SummaryCard
          ...
          caption={loading ? "Loading…" : "Persisted records"}
        />
        <SummaryCard
          label="Paid Amount"
      ...
      <Section
        ...
        subtitle="Persisted SaaS billing records owned by Cablix."
        actions={
          <FilterBar
            search={search}
            onSearch={setSearch}
        ...
        >
          {rows.map((payment) => (
            <tr key={payment.id} className="hover:bg-slate-50">
              ...
              <td className="px-4 py-3 text-slate-700">
                {payment.tenantSubscription?.plan?.name || "-"}
              </td>
