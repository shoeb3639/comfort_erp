...
export function StatusBadge({ status }) {
  const tone =
    ['ACTIVE', 'PAID', 'COMPLETED', 'CLOSED'].includes(status)
      ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20'
      : ['TRIAL', 'GRACE_PERIOD', 'PENDING_SETUP', 'PENDING', 'OWNER_INVITED'].includes(status)
  ...
  return (
    <span className={`inline-flex whitespace-nowrap rounded-full px-3 py-1 text-xs font-semibold ring-1 ring-inset ${tone}`}>
      {String(status || '-').replaceAll('_', ' ')}
    </span>
  )
}

export function SummaryCard({ label, value, caption, tone = 'default', prefix = '' }) {
  const toneClass =
    tone === 'success'
      ? 'text-emerald-700'
      : tone === 'danger'
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className={`mt-3 text-2xl font-bold ${toneClass}`}>{prefix}{value}</p>
...
export function FilterBar({ search, onSearch, placeholder = 'Search records', children }) {
  return (
...
export function TableShell({ columns, children, minWidth = '1000px', empty = false, emptyText = 'No records found.' }) {
  return (
