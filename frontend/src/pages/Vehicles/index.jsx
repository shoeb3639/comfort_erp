...
export default function VehiclesPage() {
  ...
  async function load() {
    try {
      const [vehicles, vehicleTypes, vendorRows] = await Promise.all([
        listVehicles(tab ? { ownershipType: tab } : {}),
        listVehicleTypes(),
        getVendors(),
      ]);
      setRecords(vehicles);
      setTypes(vehicleTypes);
      setVendors(vendorRows.filter((item) => item.recordType !== "own_company"));
    } catch (error) {
      setNotice({ type: "error", message: error.response?.data?.message || "Unable to load vehicles." });
    }
  }
  useEffect(() => { load(); }, [tab]);
  const visible = useMemo(
    () => records.filter((item) =>
      [item.registrationNumber, item.make, item.model, item.vendor?.name]
  ...
  async function save(event) {
    ...
    try {
      ...
      setForm(null);
      setNotice({ type: "success", message: `Vehicle ${form.id ? "updated" : "created"} successfully.` });
      await load();
    } catch (error) {
      setNotice({ type: "error", message: error.response?.data?.message || "Unable to save vehicle." });
    }
  }
  async function remove(item) {
    ...
    try {
      ...
      setNotice({ type: "error", message: error.response?.data?.message || "Unable to archive vehicle." });
    }
  }
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-2">
          {[["", "All"], ["OWN", "Own Fleet"], ["VENDOR", "Vendor Vehicles"]].map(([value, label]) => (
            <button key={label} onClick={() => setTab(value)} className={`rounded-lg px-4 py-2 text-sm font-semibold ${tab === value ? "bg-brand-600 text-white" : "bg-white text-slate-600"}`}>{label}</button>
          ))}
        </div>
        <button onClick={() => setForm({ ...empty })} className="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white"><Plus size={17} /> Add Vehicle</button>
      </div>
      ...
      {form && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><form onSubmit={save} className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-6">
        <div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-bold">{form.id ? "Edit" : "Add"} Vehicle</h2><button type="button" onClick={() => setForm(null)}><X /></button></div>
        <div className="grid gap-4 md:grid-cols-2">
          <label>Ownership<select className={field} value={form.ownershipType} onChange={(e) => setForm({ ...form, ownershipType: e.target.value, vendorId: "" })}><option value="OWN">Own</option><option value="VENDOR">Vendor</option></select></label>
          {form.ownershipType === "VENDOR" && <label>Vendor<select required className={field} value={form.vendorId} onChange={(e) => setForm({ ...form, vendorId: e.target.value })}><option value="">Select vendor</option>{vendors.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}</select></label>}
          <label>Registration Number<input required className={field} value={form.registrationNumber} onChange={(e) => setForm({ ...form, registrationNumber: e.target.value })} /></label>
        ...
        <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setForm(null)} className="rounded-lg border px-4 py-2">Cancel</button><button className="rounded-lg bg-brand-600 px-4 py-2 font-semibold text-white">Save Vehicle</button></div>
      </form></div>}
    </div>
  );
}
