...
const navItems = [
  ...
  { id: 'bookings', to: '/bookings', label: 'Bookings', shortLabel: 'Jobs', icon: CalendarCheck },
  { id: 'customers', to: '/customers', label: 'Customers', shortLabel: 'Clients', icon: Users },
  { id: 'vendors', to: '/vendors', label: 'Vendors', shortLabel: 'Vendors', icon: CarFront },
  { id: 'invoices', to: '/invoices', label: 'Invoices', shortLabel: 'Bills', icon: ReceiptText },
  {
    id: 'accounts',
    label: 'Accounts',
    shortLabel: 'Accts',
    ...
    children: [
      ...
      { to: '/accounts/transactions', label: 'Expense Entry', aliases: ['/accounts/expense-entry', '/expenses'] },
  ...
  { id: 'reports', to: '/reports', label: 'Reports', shortLabel: 'Reports', icon: BarChart3 },
  {
    id: 'settings',
    label: 'Settings',
    shortLabel: 'More',
    icon: Settings,
    children: [
      { to: '/settings', label: 'General Settings', exact: true },
...
const bottomNavItems = [
  ...
  { to: '/bookings', label: 'Bookings', shortLabel: 'Jobs', icon: CalendarCheck },
  { to: '/customers', label: 'Customers', shortLabel: 'Clients', icon: Users },
  { to: '/invoices', label: 'Invoices', shortLabel: 'Bills', icon: ReceiptText },
  { to: '/accounts/manager-ledger', label: 'Accounts', shortLabel: 'Accts', icon: WalletCards },
]
...
const pageMeta = {
  ...
  '/accounts': {
    ...
    description: 'Manage manager ledger, operational transactions, booking cash deposits, daily closing, and audit verification.',
  },
  '/expenses': {
...
function getPageMeta(pathname) {
  if (pathname === '/bookings/new') {
    return {
      eyebrow: 'Reservations',
      title: 'New Booking',
      description: 'Create a new trip reservation with billing, traveller, route, and duty details.',
    }
  }
  ...
  if (/^\/bookings\/[^/]+$/.test(pathname)) {
    return {
      eyebrow: 'Reservations',
      title: 'View Booking',
      description: 'Readonly booking summary, assignment, billing, and close status.',
    }
  }
  ...
  if (/^\/bookings\/[^/]+\/close$/.test(pathname)) {
    return {
      ...
      title: 'Close Booking',
      description: 'Close completed trip, calculate customer billing, and track vehicle profit.',
    }
  }
  ...
  if (/^\/bookings\/[^/]+\/collections$/.test(pathname)) {
    return {
      ...
      description: 'Track booking payments and company-account deposit status separately from manager expense funds.',
    }
  }
  ...
  if (/^\/bookings\/[^/]+\/profit$/.test(pathname)) {
    return {
      ...
      description: 'Per-booking profitability and vehicle impact. Recoverable charges and GST are excluded from vehicle profit.',
    }
  }
  ...
  if (pathname === '/customers/new') {
    return {
      ...
      description: 'Create a billing party for retail, corporate, or travel agent work.',
    }
  }
  ...
  if (pathname === '/invoices/new') {
    return {
      ...
      description: 'Create invoice using the common invoice template and generic line items.',
    }
  }
  ...
  if (pathname === '/accounts/manager-ledger') {
    return {
      ...
      description: 'Track each manager running balance for company fund releases, operational expenses, returns, and adjustments.',
    }
  }
  ...
  if (pathname === '/accounts/manager-ledger/release') {
    return {
      ...
      description: 'Credit manager ledger with company released operational funds. Customer collections remain separate.',
    }
  }
  ...
  if (/^\/accounts\/manager-ledger\/[^/]+$/.test(pathname)) {
    return {
      ...
      description: 'Review running manager ledger entries, credits, debits, and balance movement.',
    }
  }
  ...
  if (pathname === '/accounts/transactions') {
    return {
      eyebrow: 'Accounts',
      title: 'Transactions',
      description: 'Record all operational transactions paid from Manager Ledger. Customer collection is company revenue and never expense cash.',
    }
  }
  ...
  if (pathname === '/accounts/booking-cash-deposit') {
    return {
      ...
      description: 'Track cash received from bookings and deposit to company account. This is separate from Manager Ledger.',
    }
  }
  ...
  if (pathname === '/accounts/daily-closing') {
    return {
      eyebrow: 'Accounts',
      title: 'Daily Closing',
      description: 'Date-wise manager fund opening balance, funds received, expenses, and carry-forward balance.',
    }
  }
  ...
  if (pathname === '/accounts/audit-verification') {
    return {
      ...
      description: 'Verify and audit manager funds, booking cash deposits, balances, and operational transactions.',
    }
  }
  ...
  if (pathname === '/settings/users') {
    return {
      ...
      title: 'Users',
      description: 'Manage ERP users, branch assignments, status, and role mapping.',
    }
  }
  ...
  if (pathname === '/settings/gst-registrations') {
    return {
      ...
      description: 'Manage GST registrations, defaults, and branch mappings for the tenant.',
    }
  }
  ...
  if (pathname === '/settings/gst-registrations/new') {
    return {
      ...
      description: 'Create a GST registration for legal identity, state, and invoice-series readiness.',
    }
  }
  ...
  if (/^\/settings\/gst-registrations\/[^/]+\/edit$/.test(pathname)) {
    return {
      ...
      description: 'Update GST registration details, status, default setting, and address.',
    }
  }
  ...
  if (pathname === '/settings/permissions') {
    return {
      eyebrow: 'Access control',
      title: 'Permissions',
      description: 'Review the permission-key catalog used for future route and action guards.',
    }
  }
...
function getTopbarActions(pathname) {
  if (pathname === '/bookings') {
    return [{ to: '/bookings/new', label: 'New Booking', icon: Plus, variant: 'primary' }]
  }

  if (pathname === '/bookings/new' || /^\/bookings\/[^/]+\/edit$/.test(pathname)) {
    return [{ to: '/bookings', label: 'Back to List', icon: ArrowLeft, variant: 'secondary' }]
  }

  if (/^\/bookings\/[^/]+$/.test(pathname)) {
    const bookingId = pathname.split('/')[2]
    return [
      { to: `/bookings/${bookingId}/edit`, label: 'Edit', icon: Plus, variant: 'primary' },
      { to: '/bookings', label: 'Back to Bookings', icon: ArrowLeft, variant: 'secondary' },
    ]
  }

  if (/^\/bookings\/[^/]+\/close$/.test(pathname)) {
    return [
      { form: 'close-booking-form', type: 'submit', label: 'Close Booking', icon: Plus, variant: 'primary' },
      { to: '/bookings', label: 'Cancel', icon: ArrowLeft, variant: 'secondary' },
    ]
  }

  if (/^\/bookings\/[^/]+\/collections$/.test(pathname)) {
    return [{ to: '/bookings', label: 'Back to Bookings', icon: ArrowLeft, variant: 'secondary' }]
  }
  ...
  if (pathname === '/customers') {
    return [{ to: '/customers/new', label: 'New Customer', icon: Plus, variant: 'primary' }]
  }

  if (pathname.startsWith('/customers/')) {
    return [{ to: '/customers', label: 'Back to Customers', icon: ArrowLeft, variant: 'secondary' }]
  }

  if (pathname === '/vendors') {
    return [{ to: '/vendors?create=vendor', label: 'Add Vendor', icon: Plus, variant: 'primary' }]
  }

  if (pathname === '/invoices') {
    return [
      { to: '/invoices/settings', label: 'Series Settings', icon: Settings, variant: 'secondary' },
      { to: '/invoices/new', label: 'New Invoice', icon: Plus, variant: 'primary' },
    ]
  }
  ...
  if (/^\/invoices\/[^/]+\/preview$/.test(pathname)) {
    ...
    return [
      { to: `/invoices/${invoiceId}/print`, label: 'Print', icon: ReceiptText, variant: 'secondary' },
      { onClick: () => window.print(), label: 'Download PDF', icon: Plus, variant: 'primary' },
    ]
  }

  if (pathname.startsWith('/invoices/') && pathname !== '/invoices/settings') {
    return [{ to: '/invoices', label: 'Back to Invoices', icon: ArrowLeft, variant: 'secondary' }]
  }

  if (pathname === '/invoices/settings') {
    return [
      { form: 'invoice-settings-form', type: 'submit', label: 'Save Settings', icon: Plus, variant: 'primary' },
      { to: '/invoices', label: 'Back', icon: ArrowLeft, variant: 'secondary' },
    ]
  }

  if (pathname === '/accounts/manager-ledger') {
    return [{ to: '/accounts/manager-ledger/release', label: 'Release Amount', icon: Plus, variant: 'primary' }]
  }

  if (pathname === '/accounts/manager-ledger/release') {
    return [
      { form: 'manager-ledger-release-form', type: 'submit', label: 'Save Release', icon: Plus, variant: 'primary' },
      { to: '/accounts/manager-ledger', label: 'Cancel', icon: ArrowLeft, variant: 'secondary' },
    ]
  }

  if (/^\/accounts\/manager-ledger\/[^/]+$/.test(pathname)) {
    return [{ to: '/accounts/manager-ledger', label: 'Back to List', icon: ArrowLeft, variant: 'secondary' }]
  }

  if (pathname === '/settings/gst-registrations') {
    return [{ to: '/settings/gst-registrations/new', label: 'Add GST Registration', icon: Plus, variant: 'primary' }]
  }

  if (pathname === '/settings/gst-registrations/new' || /^\/settings\/gst-registrations\/[^/]+\/edit$/.test(pathname)) {
    return [
      { form: 'gst-registration-form', type: 'submit', label: 'Save GST Registration', icon: Plus, variant: 'primary' },
      { to: '/settings/gst-registrations', label: 'Cancel', icon: ArrowLeft, variant: 'secondary' },
    ]
  }
  ...
  if (/^\/settings\/gst-registrations\/[^/]+$/.test(pathname)) {
    ...
    return [
      { to: `/settings/gst-registrations/${registrationId}/edit`, label: 'Edit', icon: Plus, variant: 'primary' },
      { to: '/settings/gst-registrations', label: 'Back to List', icon: ArrowLeft, variant: 'secondary' },
    ]
  }
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className={collapsed ? 'hidden' : 'min-w-0'}>
        <p className="text-xs font-semibold uppercase text-brand-600">Ops Hub</p>
        <h1 className="text-base font-semibold text-slate-950">Booking Admin</h1>
      </div>
    </div>
  )
}
...
function isGroupActive(item, pathname) {
  return Boolean(item.children?.some((child) => isRouteActive(child, pathname)))
}

function findActiveGroupId(pathname) {
  const explicitMatchGroup = navItems.find((item) => item.children?.some((child) => child.match?.test(pathname)))
  if (explicitMatchGroup) return explicitMatchGroup.id

  return navItems.find((item) => item.children && isGroupActive(item, pathname))?.id || ''
}
...
function SidebarNav({ collapsed = false, className = '', onNavigate }) {
  ...
  const [openGroupId, setOpenGroupId] = useState(() => findActiveGroupId(location.pathname))
  ...
  function toggleGroup(groupId) {
    setOpenGroupId((currentGroupId) => (currentGroupId === groupId ? '' : groupId))
  }
  ...
  return (
    <nav className={`space-y-1 px-3 py-4 ${className}`}>
      {navItems.map((item) => {
        ...
        const isActive = hasChildren ? isGroupActive(item, location.pathname) : isRouteActive(item, location.pathname)
          ...
          return (
            <div key={item.id}>
              ...
              >
                ...
                {!collapsed && <ChevronDown className={`transition-transform ${isOpen ? 'rotate-180' : ''}`} size={16} strokeWidth={2.3} />}
                {collapsed && (
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between gap-2 rounded-lg px-3 py-2 text-xs font-semibold ${
                            childActive ? 'bg-brand-50 text-brand-700' : 'text-slate-400'
                          }`}
                          aria-disabled="true"
                        >
                      ...
                      <NavLink
                        ...
                        className={({ isActive: navLinkActive }) =>
                          `block rounded-lg px-3 py-2 text-xs font-semibold transition ${
                            navLinkActive || childActive ? 'bg-brand-50 text-brand-700' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'
                          }`
                        }
                      >
                        {child.label}
...
function MainLayout() {
  ...
  const topbarActions = pageTopbarAction
    ? Array.isArray(pageTopbarAction) ? pageTopbarAction : [pageTopbarAction]
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800">
      ...
      >
        ...
        <SidebarNav collapsed={isSidebarCollapsed} className="min-h-0 flex-1 overflow-y-auto" />
      ...
      >
        ...
        <SidebarNav className="min-h-0 flex-1 overflow-y-auto" onNavigate={() => setIsMenuOpen(false)} />
      </aside>

      <div className={`transition-[padding] duration-200 print:pl-0 ${isSidebarCollapsed ? 'lg:pl-20' : 'lg:pl-72'}`}>
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur print:hidden">
          <div className="flex min-h-16 items-center gap-3 px-4 py-2 sm:px-6 lg:px-8">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-medium uppercase text-slate-500">{meta.eyebrow}</p>
              <h2 className="truncate text-lg font-semibold text-slate-950 sm:text-xl">{meta.title}</h2>
            ...
            {topbarActions.map((action) => {
              ...
              if (action.to) {
                return (
                  <Link key={`${action.label}-${action.to}`} to={action.to} aria-label={action.label} className={className}>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-64 rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
                  <div className="border-b border-slate-100 px-3 py-3">
                    ...
                    <p className="mt-0.5 text-xs text-slate-500">admin@example.com</p>
                  </div>
                  <button
                    type="button"
